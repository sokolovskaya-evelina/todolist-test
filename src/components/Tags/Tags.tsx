...
const Tags = () => {
    ...
    return (
        <>
            ...
            <Tag onClick={onFilterTasksByTagId} selected={selectedTagsIds.includes(ALL)} key={ALL}
                 tag={{id: ALL, name: 'Все', color: '#BBE1FA'}}/>
            <Tag onClick={onFilterTasksByTagId} selected={selectedTagsIds.includes(WITHOUT_TAGS)} key={WITHOUT_TAGS}
                 tag={{id: WITHOUT_TAGS, name: 'Без тегов', color: '#BBE1FA'}}/>
            {Object.values(tags).map(tag => <Tag selected={selectedTagsIds.includes(tag.id)}
                                                 onClick={onFilterTasksByTagId} key={tag.id} tag={tag}
